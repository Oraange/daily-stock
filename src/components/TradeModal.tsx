...
const EMO_OPTIONS: Emotion[] = ['침착', '자신감', '불안', '조급', '욕심', '후회'];
...
export default function TradeModal({ onClose }: Props) {
  ...
  const onSave = async () => {
    ...
    if (!Number.isFinite(qtyN) || qtyN <= 0) return setError('수량을 올바르게 입력해주세요.');
    if (!Number.isFinite(buyN) || buyN <= 0) return setError('매수가를 올바르게 입력해주세요.');
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div>
              <label className="field-label" htmlFor="trade-sell">청산가 (보유 중이면 비워두세요)</label>
              <input
                id="trade-sell"
                className="field-input"
                placeholder="₩0"
          ...
          <div>
            <label className="field-label" htmlFor="trade-date">거래일</label>
            <input
              id="trade-date"
              type="date"
              className="field-input"
